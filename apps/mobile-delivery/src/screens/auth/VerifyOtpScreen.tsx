  ...
  return (
    <Screen edges={[]} style={styles.screen}>
      <View style={styles.content}>
        ...
        <Input label="Verification code" keyboardType="number-pad" maxLength={6} value={code} onChangeText={setCode} placeholder="482913" />
        {error && (
          <Text variant="caption" color="destructive">
            {error}
          </Text>
        )}
        <Button label={loading ? "Verifying…" : "Verify"} onPress={() => void onSubmit()} loading={loading} disabled={code.trim().length !== 6} />
