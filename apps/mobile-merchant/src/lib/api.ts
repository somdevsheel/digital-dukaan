...
// Coalesces concurrent 401s into one refresh call — same reasoning as mobile-customer's
// fetch wrapper (Architecture §9's refresh-token reuse detection would treat a second
// caller's rotated-out token as a replay and kill the whole session otherwise).
...
export interface ApiFetchOptions extends Omit<RequestInit, "body"> {
  ...
  /** Set false for calls made before a session exists yet (login). */
